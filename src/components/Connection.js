...
const Connection = () => {
  ...
  const ConnectWallet = async () => {
    ...
    try {
      ...
      setIsLogged(true);
      setCurrentWalletAddress(web3.utils.toChecksumAddress(accounts[0]));
      console.log();
      return accounts[0];
    } catch (err) {
      if (err.code === 4001) {
  ...
  const shortAddr = () => {
    return `${currentWalletAddress.substr(
      0,
      4
    )}...${currentWalletAddress.substring(
      currentWalletAddress.length - 4,
      currentWalletAddress.length
    )}`;
  };
  ...
  const Chain = (props) => {
    ...
    switch (chainId) {
      ...
      case 42: //KOVAN
        chainLogo = ChainLogo.eth;
        variant = "secondary";
        chainName = "Kovan Network (TestNet)";
        break;
      case 56: //BNB
