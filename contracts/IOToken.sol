 ...
 */
contract IOToken is Context, IERC20, IERC20Metadata, ERC20Burnable, ERC20Snapshot, Ownable, Pausable {
    
    // mapping associant un adresse à un montant (soldes)
    mapping(address => uint256) private _balances;
    
    // mapping listant les adresses et y associant un mapping liant une adresse à un montant alloué
    mapping(address => mapping(address => uint256)) private _allowances;

    // Nom du contrat
    string private _name= "IOToken";
    // Symbole du contrat
    string private _symbol = "PFIO";
    
    // Nombre de Jetons émis
    ...
    function _transfer(
        ...
        // Vérifie que l'expéditeur sender est valide / renseigné
        require(sender != address(0), "ERC20: transfer from the zero address");
        // Vérifie que le destinataire recipient est valide / renseigné
        require(recipient != address(0), "ERC20: transfer to the zero address");

        // Appel de fonction pour enregistrer l'état avant transfert (snapshot)
        _beforeTokenTransfer(sender, recipient, amount);

        // Enregistre le solde courant de l'expéditeur sender
        uint256 senderBalance = _balances[sender];
        // Vérifie que le solde courant de l'expéditeur sender permet la transfert du montant amount
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");

        // Défalque le montant amount transféré sur solde courant de l'expéditeur sender
        unchecked {
            _balances[sender] = senderBalance - amount;
        }
        // Crédite le montant amount transféré au solde courant du destinataire recipient
